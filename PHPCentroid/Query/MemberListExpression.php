...
class MemberListExpression extends ArrayObject
{
        ...
        if (is_array($input)) {
            foreach ($input as $item) {
                if (is_string($item))
                    ...
                    Args::check($item instanceof SelectableExpression, 'Invalid argument.Expected MemberExpression');
                    $arr[] = $item;
                }
            }
        }
    ...
    public function append($value)
    {
        if (is_string($value)) {
            ...
            Args::check($value instanceof SelectableExpression, 'Invalid argument.Expected MemberExpression');
            parent::append($value);
        }
