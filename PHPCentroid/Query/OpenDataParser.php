...
class OpenDataParser
{
    ...
    public function resolve_member($closure) {
        Args::check($closure instanceof Closure,'Expected closure');
...
//            }
    ...
    private function parse_collection($input = array()) {
        Args::check(is_array($input), 'Expected array');
    ...
    private function parse_common() {
        ...
        if ($this->at_end()) {
            ...
            Args::not_null($operator,'Operator');
            $this->move_next();
    ...
    private function parse_member() {
        ...
        Args::check($this->get_current_token() instanceof IdentifierToken,'Expected identifier.');
        $identifier = $this->get_current_token()->value_of();
        while ($this->get_next_token() && $this->get_next_token()->is_slash()) {
            $this->move_next();
            Args::check($this->get_next_token() instanceof IdentifierToken, 'Expected identifier.');
            $this->move_next();
            $identifier .= '/'.$this->get_current_token()->value_of();
        }
        $alias = NULL;
